...
import com.shindev.huaweicamera.camfilter.helper.MagicFilterFactory;
...
//        onFilterChanged();
    ...
    public void setFilter(final Global.CameraFilterType type){
        queueEvent(new Runnable() {
            ...
            public void run() {
                if (filter != null)
                    filter.destroy();
                filter = null;
                filter = MagicFilterFactory.initFilters(type, getContext());
                if (filter != null)
                    filter.init();
                onFilterChanged();
            }
